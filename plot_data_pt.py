...
price = (df.loc[:,'Price']*1.0e-3).tolist()
km    = (df.loc[:,'Km']   *1.0e-3).tolist()
...
plt.title('Search:' + min_year + ',' + brand + ',' + model, fontsize = title_font)
wc_1 = np.where(np.array(trans) == 'Manual')
wc_2 = np.where(np.array(trans) == 'Automática')
...
plt.title('Country: Portugal', fontsize = title_font)
wc_1 = np.where(np.array(gas) == 'Gasolina')
